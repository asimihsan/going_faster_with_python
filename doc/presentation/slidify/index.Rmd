...
-   `CFFI` after re-compiling `pcre` with `--enable-jit` and then using JIT gives: 7.2s, 7.4s, 6.9s
-   `CFFI` with `pcre` JIT'ing and `gcc` profiling (`-fprofile-generate`, run, `-fprofile-use`): 6.9s, 6.9s, 6.9s
...
-   CFFI with JIT `pcre` is $\frac{7.2-22.7}{22.7} \approx 68\%$ faster than the Python version.
-   CFFI with JIT `pcre` and `gcc` profiling is $\frac{6.9-22.7}{22.7} \approx 70\%$ faster than the Python version.
